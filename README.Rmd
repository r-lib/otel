...
* To instrument a package, first add the otel package to `Suggests` in
  `DESCRIPTION`.
* Then set the `otel_tracer_name` variable to your desired default tracer
  name (also default logger and meter name). See `?otel_tracer_name`.
...
impact. `otel::start_local_active_span()` calls do nothing if no telemetry
data is collected, and they do not evaluate their arguments, either. This
allows writing
```r
...
* When the concurrent computation ends, close the span manually
  with `end_span()`. (Otherwise it would be only closed at the
