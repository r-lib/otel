...
To support concurrency on a single thread, e.g. a Shiny app serving  multiple requrests at the same time, you can create multiple sessions and switch between them manually. To do this obtain a tracer with the `get_tracer()` function and use its `$start_session()`, `$activate_session()`, `$deactivate_session()`, `$finish_session()` and `$finish_all_sessions()` methods.
