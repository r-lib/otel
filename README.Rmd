...
* When instrumenting a package, set the `otel_tracer_name` variable
  to your desired default tracer name (also default logger and meter
  name). See `?otel_tracer_name`.
...
```
without worrying about the performance hit of evaluating the `attributes`
and other arguments.
...
catching errors early during the development of the instrumented project.
...
activation of the OpenTelemetry spans manually. We recommend the following
...
* Call `start_shiny_app()` from the `global.R` file, before the app
  starts up.
* Call `start_shiny_session()` from the server function, at the start of a
  new Shiny session, and pass the Shiny session object to it.
  `start_shiny_session()` creates a long lasting OpenTelemetry span in
  `session$userData$otel_span`. This span represents the Shiny session, see
  "Concurrency" above.
